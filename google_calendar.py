...
def list_google_events(creds, calendar_id, min, max):
    try:
        ...
        events_result = service.events().list(calendarId=calendar_id, timeMin=min.isoformat(),
                                              timeMax=max.isoformat(), singleEvents=True,
...
def get_calendars():
    ...
    today = datetime.datetime.combine(datetime.date.today(), datetime.time.min,  tzinfo=ZoneInfo(TIMEZONE))
